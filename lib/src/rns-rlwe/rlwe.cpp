...
RNS_RLWE_Key rlwe_alloc_RNS_key(uint64_t N, uint64_t l, incNTT ntt, double sigma){
  ...
  // compute scaling factors for encryptions
  res->delta = (uint64_t **) safe_aligned_malloc(sizeof(uint64_t*)*l);
  for (size_t i = 0; i < l; i++){
    res->delta[i] = (uint64_t *) safe_aligned_malloc(sizeof(uint64_t)*l);
    for (size_t k = 0; k < l; k++){
      const uint64_t q = ntt->ntt[k]->GetModulus();
      res->delta[i][k] = 1;
      for (size_t j = i; j < l; j++){
        const uint64_t p = ntt->ntt[j]->GetModulus();
        res->delta[i][k] = intel::hexl::MultiplyMod(res->delta[i][k], p, q);
      }
    } 
  }
  return res;
...
// RNS_RLWE_Key rlwe_new_RNS_key(uint64_t N, uint64_t l, uint64_t log_mod, incNTT ntt, double sigma){
...
RNS_RLWE_Key rlwe_new_RNS_gaussian_key(uint64_t N, uint64_t l, double key_sigma, incNTT ntt, double sigma){
  RNS_RLWE_Key res = rlwe_alloc_RNS_key(N, l, ntt, sigma);
  for (size_t j = 0; j < N; j++){
    res->s->coeffs[j] = (uint64_t)((int64_t) generate_normal_random(key_sigma));
  }
  polynomial_to_RNS(res->s_RNS, res->s);
  return res;
}
...
void rlwe_RNSc_sample_of_zero(RNSc_RLWE out, RNS_RLWE_Key key){
  ...
  polynomial_mul_RNS_polynomial((RNS_Polynomial) out->b, key->s_RNS, tmp);
...
void rlwe_scale_RNS_rlwe_addto(RNS_RLWE out, RNS_RLWE in, uint64_t scale){
  for (size_t j = 0; j < in->a->l; j++){
    const uint64_t mod = in->a->ntt->ntt[j]->GetModulus();
...
void rlwe_RNSc_mod_switch(RNSc_RLWE c, uint64_t q){
  const uint64_t p = c->a->ntt->ntt[0]->GetModulus();
...
RNSc_RLWE rlwe_new_RNSc_sample(RNS_RLWE_Key key, uint64_t m){
  ...
  const uint64_t p = res->b->ntt->ntt[0]->GetModulus();
...
RNS_RLWE rlwe_new_RNS_sample(RNS_RLWE_Key key, uint64_t * m, uint64_t p){
  ...
  for (size_t i = 0; i < res->a->l; i++) 
    qi_prod = intel::hexl::MultiplyMod(qi_prod, (res->a->ntt->ntt[i]->GetModulus())%p, p);
  for (size_t j = 0; j < key->l; j++){
    const uint64_t q = res->b->ntt->ntt[j]->GetModulus();
...
void rlwe_RNSc_encrypt_RNS(RNSc_RLWE out, RNS_RLWE_Key key, RNSc_Polynomial m){
  rlwe_RNSc_sample_of_zero(out, key);
  for (size_t j = 0; j < m->l; j++){
    const uint64_t q = out->b->ntt->ntt[j]->GetModulus();
    intel::hexl::EltwiseFMAMod(out->b->coeffs[j], m->coeffs[j], key->delta[m->l][j] ,out->b->coeffs[j], m->N, q, 1);
  }
}
...
void rlwe_RNSc_encrypt_int(RNSc_RLWE out, RNS_RLWE_Key key, uint64_t m, uint64_t l){
  ...
  for (size_t j = 0; j < l; j++){
    const uint64_t q = out->b->ntt->ntt[j]->GetModulus();
    const uint64_t m_delta = intel::hexl::MultiplyMod(key->delta[l][j], m, q);
    out->b->coeffs[j][0] = intel::hexl::AddUIntMod(out->b->coeffs[j][0], m_delta, q);
  }
}
...
void rlwe_RNSc_decrypt_RNS(RNSc_Polynomial out, RNSc_Polynomial tmp, RNS_RLWE in, RNS_RLWE_Key key){
  rlwe_RNS_phase((RNS_Polynomial) tmp, in, key);
  polynomial_RNS_to_RNSc(tmp, (RNS_Polynomial) tmp);
  for (size_t j = 0; j <  in->b->l - out->l; j++){
    polynomial_base_reduce_round_RNSc_wo_free(tmp);
  }
  polynomial_copy_RNS_polynomial((RNS_Polynomial)out, (RNS_Polynomial)tmp);
  tmp->l = in->b->l;
}
...
void rlwe_add_RNSc_sample(RNSc_RLWE out, RNSc_RLWE in1, RNSc_RLWE in2){
  polynomial_add_RNSc_polynomial(out->a, in1->a, in2->a);
...
void rlwe_sub_RNSc_sample(RNSc_RLWE out, RNSc_RLWE in1, RNSc_RLWE in2){
  polynomial_sub_RNSc_polynomial(out->a, in1->a, in2->a);
...
RNS_RLWE_KS_Key rlwe_new_RNS_ks_key(RNS_RLWE_Key out_key, RNS_RLWE_Key in_key){
  ...
  for (size_t i = 0; i < l; i++){
    ...
    const uint64_t p = res->s[i]->b->ntt->ntt[i]->GetModulus();
...
RNS_RLWE_KS_Key rlwe_new_RNS_automorphism_key(RNS_RLWE_Key key, uint64_t gen){
  ...
  RNS_RLWE_Key perm_key = rlwe_alloc_RNS_key(key->N, key->l, key->s_RNS->ntt, key->sigma);
